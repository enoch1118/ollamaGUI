    ...
    #if !os(tvOS)
        public extension IntrospectableViewType where Self == DatePickerWithCompactStyleType {
            static func datePicker(style _: Self.Style) -> Self { .init() }
        }

        #if canImport(UIKit)
            public extension iOSViewVersion<DatePickerWithCompactStyleType, UIDatePicker> {
            ...
            public extension visionOSViewVersion<DatePickerWithCompactStyleType, UIDatePicker> {
                static let v1 = Self(for: .v1)
            }

        #elseif canImport(AppKit) && !targetEnvironment(macCatalyst)
            public extension macOSViewVersion<DatePickerWithCompactStyleType, NSDatePicker> {
                @available(*, unavailable, message: ".datePickerStyle(.compact) isn't available on macOS 10.15")
                static let v10_15 = Self(for: .v10_15)
                static let v10_15_4 = Self(for: .v10_15_4)
