...
class ChatUsecase {
    ...
    func chat(
        req: ChatRequestModel,
        appSetting: AppSetting,
        options: RoomOptionEntity?
    ) -> CurrentValueSubject<Loadable<ChatModel, NetworkError>, Never> {
        var dto = req
        dto.model = appSetting.model
    ...
    func check() async -> Future<Bool, Never> {
        ollamaRepository.check()
    }
}
