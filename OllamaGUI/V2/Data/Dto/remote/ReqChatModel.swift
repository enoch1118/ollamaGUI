...
struct ReqChatModel: Encodable {
    ...
    mutating func applyOption(option: LocalRoomOptionModel?, appSetting: AppSetting) {
        let option = option ?? LocalRoomOptionModel()
        options = .init(topP: option.top_p, topK: option.top_k, temperature: option.temperature)
        model = option.model ?? appSetting.model
    }
}
